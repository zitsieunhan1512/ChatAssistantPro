package app.mbl.hcmute.base.googleBilling
...
class BillingDataSource private constructor(
    ...
    override fun onBillingSetupFinished(billingResult: BillingResult) {
        Timber.d("onBillingSetupFinished: entry")
    ...
    private fun retryBillingServiceConnectionWithExponentialBackoff() {
        ...
        reconnectMilliseconds = min(
            reconnectMilliseconds * 2, RECONNECT_TIMER_MAX_TIME_MILLISECONDS
        )
    }
    ...
    fun getSkuTitle(sku: String): Flow<String> {
        ...
        return skuDetailsFlow.mapNotNull { skuDetails ->
            Timber.d("getSkuTitle: $skuDetails")
            skuDetails?.title
        }
    }
    ...
    fun getSkuPrice(sku: String): Flow<String> {
        val skuDetailsFlow = skuDetailsMap[sku]!!
        return skuDetailsFlow.mapNotNull { skuDetails ->
            Timber.d("getSkuPrice: %s", skuDetails?.oneTimePurchaseOfferDetails?.formattedPrice)
            Timber.d("getSkuPrice: %s", skuDetails?.subscriptionOfferDetails?.first()?.pricingPhases?.pricingPhaseList?.first()?.formattedPrice)
    ...
    fun getSkuDescription(sku: String): Flow<String> {
        val skuDetailsFlow = skuDetailsMap[sku]!!
        return skuDetailsFlow.mapNotNull { skuDetails ->
            Timber.d("getSkuTitle: %s", skuDetails?.description)
    ...
    private fun onSkuDetailsResponse(
        ...
        Timber.d("onSkuDetailsResponse: entry")
        val responseCode = billingResult.responseCode
        val debugMessage = billingResult.debugMessage
        when (responseCode) {
            BillingClient.BillingResponseCode.OK -> {
                ...
                if (skuDetailsList == null || skuDetailsList.isEmpty()) {
                    ...
                    for (skuDetails in skuDetailsList) {
                        ...
                        detailsMutableFlow?.tryEmit(skuDetails) ?: Timber.e("Unknown sku: $sku")
                    }
                }
            }
            ...
            BillingClient.BillingResponseCode.USER_CANCELED -> Timber.i("onSkuDetailsResponse: $responseCode $debugMessage")
    ...
    private suspend fun querySkuDetailsAsync() {
        if (!knownInappSKUs.isNullOrEmpty()) {
            val inAppProductList = knownInappSKUs.map {
            ...
            onSkuDetailsResponse(
                skuDetailsResult.billingResult, skuDetailsResult.productDetailsList
            )
        }
        if (!knownSubscriptionSKUs.isNullOrEmpty()) {
            val subscriptionProductList = knownSubscriptionSKUs.map {
     ...
     * @param ProductType sku type, inapp or subscription, to get purchase information for.
     * @return purchases
     */
    private suspend fun getPurchases(skus: Array<String>, ProductType: String): List<Purchase> {
    ...
    suspend fun consumeInAppPurchase(sku: String) {
        val pr = billingClient.queryPurchasesAsync(
            QueryPurchasesParams.newBuilder().setProductType(BillingClient.ProductType.INAPP).build()
        )
    ...
    private fun setSkuState(sku: String, newSkuState: SkuState) {
        val skuStateFlow = skuStateMap[sku]
        skuStateFlow?.tryEmit(newSkuState) ?: Timber.e("Unknown SKU $sku Check to make sure SKU matches SKUS in the Play developer console.")
    }
            ...
            for (purchase in purchases) {
                ...
                if (purchaseState == Purchase.PurchaseState.PURCHASED) {
                    if (!isSignatureValid(purchase)) {
                        Timber.e("Invalid signature. Check to make sure your public key is correct.")
                        continue
                    }
                    ...
                    defaultScope.launch {
                        for (sku in purchase.products) {
                            if (knownAutoConsumeSKUs.contains(sku)) {
                                ...
                                if (isConsumable) {
                                    Timber.e("Purchase cannot contain a mixture of consumable and non-consumable items: ${purchase.products}")
                                    isConsumable = false
                                    break
                                }
                            }
                        ...
                        if (isConsumable) {
                            ...
                            if (billingResult.responseCode != BillingClient.BillingResponseCode.OK) {
                                Timber.e("Error acknowledging purchase: ${purchase.products.toString()}")
                            } else {
                                // purchase acknowledged
                                for (sku in purchase.products) {
                                    setSkuState(
                                        sku, SkuState.SKU_STATE_PURCHASED_AND_ACKNOWLEDGED
                                    )
                                }
                            }
                            newPurchaseFlow.tryEmit(purchase.products)
    ...
    fun launchBillingFlow(activity: Activity?, sku: String, vararg upgradeSkusVarargs: String) {
        ...
        if (null != skuDetails) {
            ...
            val params = BillingFlowParams.ProductDetailsParams.newBuilder().setProductDetails(skuDetails)
            val productDetailsParamsList = listOf(
                if (!offerToken.isNullOrEmpty()) {
                    params.setOfferToken(offerToken).build()
                } else {
                    params.build()
                }
            )
                ...
                when (heldSubscriptions.size) {
                    1 -> {
                        ...
                        billingFlowParams.setSubscriptionUpdateParams(
                            BillingFlowParams.SubscriptionUpdateParams.newBuilder().setOldPurchaseToken(purchase.purchaseToken).setReplaceProrationMode(
    ...
    private fun isSignatureValid(purchase: Purchase): Boolean {
        return app.mbl.hcmute.base.googleBilling.Security.verifyPurchase(purchase.originalJson, purchase.signature)
    }
    ...
    companion object {
        ...
        fun getInstance(
            ...
            sInstance ?: BillingDataSource(
                application, defaultScope, knownInappSKUs, knownSubscriptionSKUs, autoConsumeSKUs
            ).also { sInstance = it }
        }
    }
